...
import de.tu_berlin.coga.zet.model.exception.AreaNotInsideException;
import de.tu_berlin.coga.zet.model.exception.PolygonNotClosedException;
import de.tu_berlin.coga.zet.model.exception.RoomEdgeInvalidTargetException;
import de.tu_berlin.coga.zet.model.exception.TeleportEdgeInvalidTargetException;
import de.tu_berlin.coga.zet.model.exception.TeleportEdgeNotConnected;
import de.tu_berlin.coga.zet.model.exception.TeleportEdgeTargetLengthException;
...
public class RoomImpl extends PlanPolygon<RoomEdge> implements Cloneable, Comparable<Room>, Room {
	@XStreamAsAttribute()
	...
	private ArrayList<AreaImpl>[] areas;
	 ...
	 */
	RoomImpl( Floor floor, String name ) {
		...
		areas = new ArrayList/*<? extends Area>*/[] {assignmentAreas, barriers, delayAreas, evacuationAreas, inaccessibleAreas, saveAreas, stairAreas, teleportAreas};
		floor.addRoom( this );
	}

  /**
   * Returns the position and the width of all doors for a {@code Room} that connects that room with a room on the same
   * floor
  ...
  public Collection<PlanEdge> getDoorEdges() {
    Collection<PlanEdge> doorEdges = new HashSet<>();
    for( RoomEdge edge : super.getEdges() ) {
      if( edge.isPassable() ) {
        doorEdges.add( edge );
      }
	...
	public PlanPolygon<RoomEdge> splitClosedPolygon( RoomEdge edge1, RoomEdge edge2 ) throws IllegalArgumentException {
		RoomImpl newPolygon = (RoomImpl) super.splitClosedPolygon( edge1, edge2 );

		for( Area a : getAreas() ) {
			if( newPolygon.getPolygon().contains( (AreaImpl)a ) ) {
				((AreaImpl)a).setAssociatedRoom( newPolygon );
			}
	...
	protected PlanPolygon<RoomEdge> createPlainCopy() {
		return new RoomImpl( getAssociatedFloor(), getName() );
	}
	...
	void rasterize() {
		super.rasterize();
		for( AreaImpl a : assignmentAreas )
			a.rasterize();
		for( AreaImpl a : barriers )
			a.rasterize();
		for( AreaImpl a : delayAreas )
			a.rasterize();
		for( AreaImpl a : evacuationAreas )
			a.rasterize();
		for( AreaImpl a : inaccessibleAreas )
			a.rasterize();
		for( AreaImpl a : saveAreas )
			a.rasterize();
		for( AreaImpl a : stairAreas )
			a.rasterize();
		for( AreaImpl a : teleportAreas )
			a.rasterize();
	}
	 ...
	 * @throws TooManyPeopleException If this room contains too many people.
	...
	public void check( boolean rasterized ) throws PolygonNotClosedException, AreaNotInsideException, TeleportEdgeInvalidTargetException, TooManyPeopleException {
		...
		for( RoomEdge e : getEdges() ) {
			if( e instanceof TeleportEdge ) {
				...
				if( e2.getLinkTarget() == null ) {
					throw new TeleportEdgeNotConnected( e2, ZLocalization.loc.getString( "ds.z.TeleportEdge.LinkTargetNotSet" ) );
				} else if( e2.getLinkTarget().getLinkTarget() != e2 ) {
					throw new TeleportEdgeNotConnected( e2, ZLocalization.loc.getString( "ds.z.TeleportEdge.InconsistentLinkage" ) );
				}
				...
				if( r.getAssociatedFloor() == associatedFloor ) {
					throw new TeleportEdgeInvalidTargetException( e2,
									ZLocalization.loc.getString( "ds.z.TeleportEdge.SameFloorException" ) );
				}
				if( e.length() != e2.length() ) {
					throw new TeleportEdgeTargetLengthException( e2,
									ZLocalization.loc.getString( "ds.z.TeleportEdge.LengthNotMatchException" ) + "(" + e2 + ", " + e + ")" );
				}
			} else {
        if( e.isPassable() ) {
          if( e.getLinkTarget() == null ) {
  					throw new RoomEdgeInvalidTargetException( e, ZLocalization.loc.getString( "ds.z.TeleportEdge.LinkTargetNotSet" ) );
          }
          if( e.getLinkTarget().getRoom() == null ) {
            System.err.println( "Room: " + this );
            System.err.println( "Edge: " + e );
  					//throw new RoomEdgeInvalidTargetException( e, ZLocalization.loc.getString( "ds.z.TeleportEdge.LinkTargetNotSet" ) );
            // fix: set passable = false
            // set link target = false
            e.setLinkTarget( null );
          }
          
          if( e.isPassable() && e.getLinkTarget().getRoom().getAssociatedFloor() != associatedFloor ) {
            throw new RoomEdgeInvalidTargetException( e, ZLocalization.loc.getString( "ds.z.Room.DifferentFloorException" ) );
          }
        }
			}
		}
		for( AssignmentArea aa : assignmentAreas ) {
			aa.check( rasterized );
			if( !(contains( aa )) ) {
				throw new AreaNotInsideException( this, aa, ZLocalization.loc.getString( "ds.z.Room.NotCompletelyInException" ) );
			}
		}
		for( Barrier b : barriers ) {
			b.check( rasterized );
			if( !(contains( b )) ) {
				throw new AreaNotInsideException( this, b, ZLocalization.loc.getString( "ds.z.Room.NotCompletelyInException" ) );
			}
		}
		for( DelayArea da : delayAreas ) {
			da.check( rasterized );
			if( !(contains( da )) ) {
				throw new AreaNotInsideException( this, da, ZLocalization.loc.getString( "ds.z.Room.NotCompletelyInException" ) );
			}
		}
		for( InaccessibleArea ia : inaccessibleAreas ) { // including barriers
			ia.check( rasterized );
			if( !(contains( ia )) ) {
        System.err.println( "Inacessible area!" );
        System.err.println( associatedFloor );
        System.err.println( "Room: " + this );
        System.err.println( ia );
        deleteArea( ia );
				throw new AreaNotInsideException( this, ia, ZLocalization.loc.getString( "ds.z.Room.NotCompletelyInException" ) );
			}
		}
		for( SaveArea sa : saveAreas ) { // Including evacuation areas
			sa.check( rasterized );
			if( !(contains( sa )) ) {
				throw new AreaNotInsideException( this, sa, ZLocalization.loc.getString( "ds.z.Room.NotCompletelyInException" ) );
			}
		}
		for( StairArea sa : stairAreas ) {
			sa.check( rasterized );
			if( !(contains( sa )) ) {
        System.err.println( "Stair area!" );
        System.err.println( associatedFloor );
        System.err.println( "Room: " + this );
		...
		for( TeleportArea ta : teleportAreas ) {
			ta.check( rasterized );
			if( !(contains( ta )) ) {
				throw new AreaNotInsideException( this, ta, ZLocalization.loc.getString( "ds.z.Room.NotCompletelyInException" ) );
			}
		}
	...
	private void checkTooManyPersonsInRoom() throws TooManyPeopleException {
		...
		if( persons != 0 && (area() / persons) < Assignment.spacePerPerson ) {
			throw new TooManyPeopleException( this, ZLocalization.loc.getString( "ds.z.Room.OverfullRoomException" ) );
		}
	}
	...
	public ArrayList<RoomEdge> replaceEdge( RoomEdge e, List<PlanPoint> p ) {
		if( e.isPassable() ) {
			...
			try {
				...
				((RoomImpl)partner.getRoom()).replaceEdgeInternal( partner, p );
	 ...
	 * <table summary="Table of different cases for edge combining.">
 ...
 which are not neighbors (only possible for TeleportEdges) or if two RoomEdgeAAs shall be combined
 who point to different rooms with their link targets.
			...
			if( e1.isPassable() && e2.isPassable() ) {
				...
				if( !e1.getLinkTarget().getRoom().equals( e2.getLinkTarget().getRoom() ) ) {
					...
					if( !teleportEdges && e1.equals( e2 ) ) {
						...
						throw new IllegalArgumentException( ZLocalization.loc.getString( "ds.z.Room.CombinePassablesToDifferentRooms" ) );
					}
				} else {
					// Combine passables to a single link-target room
					RoomImpl target = (RoomImpl)e1.getLinkTarget().getRoom();
					// Stop here if the operation will fail because of the target size
					if( keepMinSize && target.getPolygon().getNumberOfEdges() - 1 < 3 ) {
						throw new IllegalStateException( ZLocalization.loc.getString( "ds.z.Room.TargetNotEnoughEdgesException" ) );
					}
	 ...
	 * <p>The {@code RoomEdgeAA} is associated to both rooms and is setLocation as passable.
	 * This creates a "door" between the two rooms.</p>
	 * @param r the other room
	 * @param e the edge
	 */
	void connectTo( RoomImpl r, RoomEdge e ) {
		// Connects this room to a specified room at the edge e)
		RoomEdge e1 = getEdge( e );
		RoomEdge e2 = r.getEdge( e );
		if( e1.isPassable() || e2.isPassable() ) {
			throw new IllegalStateException( ZLocalization.loc.getString( "ds.z.Room.NoMoreConnectionsPossibleException" ) );
		}
		e1.setLinkTarget( e2 );
		e2.setLinkTarget( e1 );
	}
	 ...
	 */
	void connectTo( RoomImpl r, PlanPoint p1, PlanPoint p2 ) {
		RoomEdge e1 = getEdge( p1, p2 );
		RoomEdge e2 = r.getEdge( p1, p2 );
		if( e1.isPassable() || e2.isPassable() ) {
			throw new java.lang.IllegalStateException( ZLocalization.loc.getString( "ds.z.Room.NoMoreConnectionsPossibleException" ) );
		}
		e1.setLinkTarget( e2 );
		e2.setLinkTarget( e1 );
	}
	 ...
	 * @throws IllegalArgumentException If both edges are in the same room or
 ...
 the deletion of a RoomEdgeAA (source/linkTarget) may fail.
    ...
    if( targetEdge.getRoom() == sourceEdge.getRoom() ) {
      throw new IllegalArgumentException( ZLocalization.loc.getString( "ds.z.Room.NoDifferentRoomsException" ) );
    }
    if( targetEdge.getRoom().getAssociatedFloor() == sourceEdge.getRoom().getAssociatedFloor() ) {
      throw new IllegalArgumentException( ZLocalization.loc.getString( "ds.z.Room.NoDifferentFloorsException" ) );
    }
	...
	void deleteArea( Area area ) throws IllegalArgumentException {
		...
		if( !result ) {
			throw new IllegalArgumentException( ZLocalization.loc.getString( "ds.z.Room.NoAreaException" ) );
		}
	}
	...
	public List<Area> getAreas() {
		int size = 0;
		for( ArrayList<AreaImpl> areaList : areas )
			size += areaList.size();
	...
	public void setAssociatedFloor( Floor floor ) throws IllegalArgumentException {
		try {
			if( floor != null ) {
				...
				throw new IllegalArgumentException( ZLocalization.loc.getString( "ds.z.Room.NoFloorException" ) );
			}
...
if( inaccessibleAreas == null )
	 ...
	 */
	public void repair() {
		for( ArrayList<AreaImpl> testList : areas ) {
			for( Area area : testList )
				if( area == null ) { // check, if an area is a problem
					ArrayList<AreaImpl> workingList = new ArrayList<>();
	...
	@Override
  public PlanPolygon<RoomEdge> getPolygon() {
    return this;
  }
