...
/**
 * The central project class for the Z format. All information about the evacuation scenario is linked together in this
 * class.
 */
...
public class Project implements Serializable {
    ...
    private final ArrayList<EvacuationPlan> evacuationPlans;
    /** Additionally stored visualization parameters for the project. */
    private VisualProperties vp;

    public Project() {
        plan = new BuildingPlan();
        assignments = new ArrayList<>(2);
        evacuationPlans = new ArrayList<>(0);
        vp = new VisualProperties();
    }

    /**
     * Adds a new EvacuationPlan to the Project. If the Project hasn't had a {@code CurrentEvacuationPlan} previously,
     * then the new EvacuationPlan is made the current one.
     *
     ...
     */
    void addEvacuationPlan(EvacuationPlan val) throws IllegalArgumentException {
        if (evacuationPlans.contains(val)) {
            throw new IllegalArgumentException(ZLocalization.loc.getString("ds.PlanAlreadyExistsException"));
        }
    ...
    /**
     * Deletes an EvacuationPlan from the Project. If you deleted the "current" EvacuationPlan, the first EvacuationPlan
     * in the List of the remaining ones will be selected to become "current", if the List is empty, the
     * {@code currentEvacuationPlan} will be set to null.
     *
     * @param val
     * @throws IllegalArgumentException if the EvacuationPlan {@code val} is not registered at the Project.
     */
    void deleteEvacuationPlan(EvacuationPlan val) throws IllegalArgumentException {
        if (!evacuationPlans.remove(val)) {
            throw new IllegalArgumentException(ZLocalization.loc.getString("ds.PlanNotRegisteredException"));
        }
        val.delete();
     ...
     *
     * @param val
     * @throws IllegalArgumentException if {@code val} is not in the list of EvacuationPlans that is maintained by the
     * Project.
     */
    void setCurrentEvacuationPlan(EvacuationPlan val) throws IllegalArgumentException {
        if (!evacuationPlans.contains(val)) {
            throw new IllegalArgumentException(ZLocalization.loc.getString("ds.PlanNotInProjectException"));
        }
        this.currentEvacuationPlan = val;
    }

    /**
     * Adds a new Assignment to the Project. If the Project hasn't had a "CurrentAssignment" previously, then the new
     * Assignment is made the current one.
     *
     * @param val
     * @throws IllegalArgumentException if the Assignment {@code val} is already registered at the Project.
     */
    void addAssignment(Assignment val) throws IllegalArgumentException {
        if (assignments.contains(val)) {
            throw new IllegalArgumentException(ZLocalization.loc.getString("ds.AssignmentAlreadyRegisteredException"));
        }
    ...
    /**
     * Deletes an Assignment from the Project. If you deleted the "current" Assignment, the first Assignment in the List
     * of the remaining ones will be selected to become "current", if the List is empty, the {@code currentAssignment}
     * will be set to {@code null}.
     *
     * @param val
     * @throws IllegalArgumentException - Is thrown when the Assignment {@code val} is not registered at the Project.
     */
    void deleteAssignment(Assignment val) throws IllegalArgumentException {
        if (!assignments.remove(val)) {
            throw new IllegalArgumentException(ZLocalization.loc.getString("ds.AssignmentNotRegisteredException"));
        }
        if (val == null) {
            return;
        }
    ...
    /**
     * Sets the Assignment that is to be used. Gives notice to the previous and the new currentAssignment that it is not
     * any more respectively now the current assignment.
     *
     * @param val
     * @throws IllegalArgumentException if {@code val} is not in the list of Assignments that is maintained by the
     * Project.
     */
    void setCurrentAssignment(Assignment val) throws IllegalArgumentException {
        if (!assignments.contains(val)) {
            throw new IllegalArgumentException(ZLocalization.loc.getString("ds.AssignmentNotInProjectException"));
        }
    ...
    public boolean equals(Object o) {
        if (o instanceof Project) {
            ...
            return (subobjects_me.isEmpty() && subobjects_p.isEmpty());
        } else {
            return false;
        }
    }
