...
/**
 * A {@code DefaultEvacuationFloor} basically behaves as a normal {@link Floor}. It can create automatically designed
 * "evacuation rooms" that are rooms containing only one evacuation area that are connected to another floor with an
 * {@link TeleportEdge}. It is possible to use the normal methods to edit the floor, but it is recommended only to use
 * only the auto creating functison since they normally do not check wheter there where manual edits.
 * <p>
 * The default behaviour creates quadratic rooms of the same size arranged to a square. This allows comfortable viewing
 * the floor, but does not check for incompatibilities to possibly changes. The behaviour can be changed, so that the
 * new rooms are added in a long line of rooms, just outside the currently used area. This is recommended if direct
 * changes to the floor structure are applied. </p>
 * <p>
 * Warning! Once changed the behaviour, it cannot be changed back to default behaviour. The created rooms in default
 * mode have a defined size, which can be changed to allow wider teleport edges. </p>
 *
 * @see Floor
...
public class DefaultEvacuationFloor extends Floor {
  ...
  /**
   * Creates a new instance of {@code DefaultEvacuationFloor} with the default name "EvacuationFloor".
   */
  public DefaultEvacuationFloor() {
    super();
    this.setName( ZLocalization.loc.getString( "ds.z.DefaultName.EvacuationFloor" ) );
  }
  ...
  /**
   * Creates a new room that will be connected to the sourceEdge by calling Room.connectToWithTeleportEdge after having
   * created the new room. The sourceEdge has to be aligned horizontally or vertically.
   *
  ...
  public void addEvacuationRoom( RoomEdgeInterface sourceEdge ) {
    ...
    if( !(sourceEdge.isHorizontal() || sourceEdge.isVertical()) ) {
      throw new java.lang.IllegalArgumentException( ZLocalization.loc.getString( "ds.z.DefaultEvacuationFloor.EdgeNotVerticalOrHorizontalException" ) );
    }
    if( sourceEdge.length() % rasterSize != 0 ) {
      throw new java.lang.IllegalArgumentException( ZLocalization.loc.getString( "ds.z.DefaultEvacuationFloor.EdgeLengthRasterException" ) );
    }
    ...
    RoomImpl newRoom = new RoomImpl( this, ZLocalization.loc.getString( "ds.z.DefaultName.EvacuationRoom" ) + " " + Integer.toString( roomCount ) );
    ...
    List<PlanPoint> points = new ArrayList<>( 6 );
    points.add( ul );
    points.add( ur );
    ...
    PlanEdge door = newRoom.getEdge( p1, p2 );
    ...
    RoomImpl.connectToWithTeleportEdge( sourceEdge, (RoomEdge) door );
  }
  ...
  private String getNewEvacuationAreaName() {
    String base = ZLocalization.loc.getString( "ds.z.DefaultName.DefaultEvacuationArea" );
  ...
  /**
   * Sets the default size for new rooms. In normal mode all new rooms will have exactly the size defined by this value.
   * In simple mode all rooms have at least the specified size. Already created rooms are not touched if the value is
   * changed.
   *
   * @param size the default size
   * @throws java.lang.IllegalArgumentException if the size is less than the former size in normal mode or if it does
   *                                            not fit to the raster.
   */
  ...
  /**
   * Sets the raster size to the specified value. The rooms already created are not aligned to the new raster.
   *
  ...
  public void setRasterSize( int size ) {
    if( size <= 0 ) {
      throw new java.lang.IllegalArgumentException( ZLocalization.loc.getString( "ds.z.DefaultEvacuationFloor.RasterNegativeException" ) );
    }
    rasterSize = size;
  }
